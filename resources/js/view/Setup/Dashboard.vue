<template>
    ...
    <div class="p-4 sm:ml-64 mt-16">
        ...
        <div class="justify-between items-center w-full mb-3">
        ...
        <div class="grid md:grid-cols-3 md:gap-2 mb-10 w-full">
                <div v-for="cash in cashtypeAndRemittance" :key="cash.cash_remittance.CASH_TYPE_ID">
                    <div class="max-w-sm p-6 bg-white border border-gray-200 rounded-sm shadow-md shadow-yellow-300/50 dark:bg-gray-800 dark:border-gray-700 mb-3">
                        <div class="flex flex-col justify-center items-center">
                            <div class="flex justify-start items-center w-full mb-3">
                                ...
                                <h1 class="text-slate-500 text-1xl font-bold ml-10">{{cash.DESCRIPTION}} Balance</h1>
                            </div>
                            <h1 class="text-slate-500 font-bold text-2xl text-sky-500">P{{cash.cash_remittance.AMOUNT}}</h1>
                        </div>
                    </div>
                </div>
                ...
                <div class="max-w-sm p-6 bg-white border border-gray-200 rounded-lg shadow-md shadow-rose-500/50 dark:bg-gray-800 dark:border-gray-700 mb-3">
                    <div class="flex flex-col justify-center items-center">
                        ...
                        <h1 class="text-slate-500 font-bold text-2xl text-sky-500">{{MonthlyExpenses}}</h1>
                    </div>
                </div>
        </div>
        ...
        <div class="justify-between items-center w-full mb-3">
            <h1 class="text-1xl font-bold text-slate-500">Add Expense</h1>
        </div>
        ...
        <div class="md:flex justify-between items-center w-full mb-3">
            <div>
            ...
            <div class="text-md flex justify-between items-center">
                <div class="flex flex-col mr-5">
                    ...
                    <select v-model="formData.sort"
                        ...
                        <option v-for="expense in Expenses" :key="expense.EXPENSE_ID" :value="expense.EXPENSE_ID">
                            {{ expense.DESCRIPTION }}
                        </option>
                    </select>
                ...
                <tbody>
                        <tr v-for="expenses in UserExpenses" :key="expenses.EXPENSE_TYPE" class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                            <th scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                {{expenses.cash_type ? expenses.cash_type.DESCRIPTION : ''}}
                            </th>
                            <td class="px-6 py-4">
                                {{expenses.expenses_type ? expenses.expenses_type.DESCRIPTION : ''}}
                            </td>
...
<script>
    ...
    export default {
        components: {mainLayout,GoogleChart,modal,SuccessMessage,ErrorMessage},
        data() {
            return {
                chartData: [
                    ['Month', 'Income', 'Amount'],
        ...
        methods:{
            ...
            fetchTransactionDate(date){
                ...
                post(`UserExpense/getCurrentUserExpenseBaseOnDate`,FormData)
                    .then(response => this.UserExpenses =  response.data)
                    .catch(error => {
                        this.hasError = true;
                        this.ErrorMessage.push(error.response.data.message)
            ...
            SortTransaction(sort){
                ...
                post('UserExpense/SortCurrentUserExpense',FormData)
                    .then(response => this.UserExpenses = response.data)
